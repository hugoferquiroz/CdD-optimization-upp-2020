...
program define resultado_meta 
	...
	label define resultado 1 "Ganador" 0 "No alcanzó la meta"
...
merge 1:m codplaza using `nexus', keepusing(descargo) keep(1 3)

*export excel dre ugel nivel cod_mod_origen tipodeplaza codplaza rddereubicacion using "Plazas que no se encuentran en Nexus.xlsx" if _merge==1 & (tipodeplaza=="DOCENTE" | tipodeplaza=="PROFESOR" | tipodeplaza=="EVENTUAL"), sheet("810-2019") sheetreplace

drop if _merge==1
drop _* 

keep if strpos(d_gestion_destino, "Pública") & !mi(nivel_origen) & !mi(nivel_destino)
...
*export excel dre ugel nivel cod_mod_origen tipodeplaza codplaza rddereubicacion using "Plazas que no se encuentran en Nexus.xlsx" if _merge==1 & (tipodeplaza=="DOCENTE" | tipodeplaza=="PROFESOR" | tipodeplaza=="EVENTUAL"), sheet("869-2019") sheetreplace
...
foreach noficio in 1014 998 47 {
	...
	*export excel dre ugel nivel cod_mod_origen tipodeplaza codplaza rddereubicacion using "Plazas que no se encuentran en Nexus.xlsx" if _merge==1 & (tipodeplaza=="DOCENTE" | tipodeplaza=="PROFESOR" | tipodeplaza=="EVENTUAL"), sheet("`oficio'-2019") sheetreplace
